...
function makeEnv(profile, sizing, secrets, selected) {
  ...
  const values = [
    ...
    ...(profile.target.runtime === "podman" ? [] : [["DOCKER_SOCKET_PATH", "/var/run/docker.sock"]]),
...
["START_ADDITIONAL_SERVICES", [...notificationServices, ...aiServices].join(",")],
    ...
    ["SMTP_SENDER_DISPLAY_NAME", profile.mail?.senderDisplayName ?? ""],
    ...
    ["SMTP_TRANSPORT_SECURITY", profile.mail?.transportSecurity ?? ""],
    ["SMTP_CA_TRUST", profile.mail?.caTrust ?? ""],
